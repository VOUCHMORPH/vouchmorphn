...
class SwapService
{
    ...
    private function executeSignedCashout(array $payload): array
    {
        ...
        $authId = $this->storeCashoutAuthorization(
            ...
            $generateResult['swap_code'] ?? $generateResult['voucher_number'],
        ...
        $result = [
            ...
            'auth_id' => $authId,
            'swap_code' => $generateResult['swap_code'] ?? $generateResult['voucher_number'],
    ...
    private function storeCashoutAuthorization(
        ...
        try {
            ...
            $stmt->execute([
                ...
                ':source_wallet' => null, // Can be populated if needed
                ':amount' => $amount,
                ':currency' => $this->config['currency'] ?? 'BWP',
                ...
                ':cashout_point' => 'ATM', // Default, can be AGENT
    ...
    public function confirmCashout(array $payload): array
    {
        ...
        $completionResult = $this->feeEngine->calculateStandardCompletion($actualDeliveredAmount ?: $amountToSend);
        ...
        // STEP 5: UPDATE CASHOUT AUTHORIZATION STATUS
        $this->updateCashoutAuthorizationStatus($authorization['auth_id'], 'COMPLETED', $cashoutPoint);
        
        // STEP 6: RECORD SETTLEMENT (DELEGATE TO HYBRID SETTLEMENT)
        ...
        // STEP 7: INVOICE FEES (DELEGATE TO HYBRID SETTLEMENT)
    ...
    public function handleExpiredCashout(string $swapReference): array
    {
        ...
        $authorization = $this->getCashoutAuthorization($swapReference, null);
        
        if (!$authorization) {
            throw new RuntimeException("No cashout authorization found for reference: {$swapReference}");
        }
        
        if ($authorization['code_expiry'] > date('Y-m-d H:i:s')) {
        ...
        $this->updateCashoutAuthorizationStatus($authorization['auth_id'], 'EXPIRED');
        
        // Record settlement
        $this->settlement->updateNetPosition(
            $swapReference,
            $authorization['source_institution'],
            'VOUCHMORPH',
