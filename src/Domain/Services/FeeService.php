 ...
 */
public function calculateFeesWithDetails(
    float $amount, 
    string $currency, 
    string $cardType,
    ?array $metadata = null
    ...
    // Calculate base fees
    $baseFees = $this->calculateFees($amount, $currency);
    
    // Add card-specific fees
    $cardFees = $this->calculateCardFees($amount, $cardType);
    
    // Calculate total
    $totalFees = $baseFees + $cardFees;
    ...
    return [
        ...
        'breakdown' => [
            ...
            'network_fee' => $this->calculateNetworkFee($amount, $cardType),
            'currency_conversion' => $this->calculateCurrencyConversionFee($amount, $currency)
        ],
        'total_fees' => $totalFees,
        'net_amount' => $amount - $totalFees,
        'fee_percentage' => round(($totalFees / $amount) * 100, 2),
        'metadata' => $metadata
    ];
}

/**
 * Calculate card-specific fees
 */
private function calculateCardFees(float $amount, string $cardType): float
{
    $feeRates = [
        'VISA' => 0.025,   // 2.5%
    ...
    $rate = $feeRates[strtoupper($cardType)] ?? 0.03; // Default 3%
    return $amount * $rate;
}
...
{
    return max(0.30, $amount * 0.005); // $0.30 or 0.5%
}

/**
 * Calculate network fee
 */
private function calculateNetworkFee(float $amount, string $cardType): float
{
    $networkRates = [
        'VISA' => 0.001,
        'MASTERCARD' => 0.001,
    ...
    $rate = $networkRates[strtoupper($cardType)] ?? 0.001;
    return $amount * $rate;
}
 ...
 */
private function calculateCurrencyConversionFee(float $amount, string $currency): float
{
    $conversionRates = [
        'USD' => 0.0,
        'EUR' => 0.01,
    ...
    $rate = $conversionRates[strtoupper($currency)] ?? 0.02;
    return $amount * $rate;
}
