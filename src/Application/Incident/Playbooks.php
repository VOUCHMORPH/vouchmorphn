...
final class Playbooks
{
    ...
    public static function rules(): array
    {
        return [
            ...
            'FEE_IMBALANCE' => ['SEV2', 'S6', false, ['ACCOUNTANT', 'HEAD_OF_PRODUCTS'],
                'A transaction charged the customer more or less than the sum of the fee shares recorded for the parties. Open Accounts, Fee ledger, find the swap, and establish which party is missing a share or holds one twice. Correct by recording the missing share or reversing the wrong one - never by editing a recorded row.'],
            'JOBS_STALLED' => ['SEV2', 'S3', false, ['TECH_RECOVERY_LEAD', 'HEAD_OF_PRODUCTS'],
                'The scheduled jobs have stopped: holds are not being released, settlement advices are not going out and alarms may be silent. Check the jobs service is running and its hosting subscription is paid, then confirm the next run completes.'],
            'RETURN_OVERDUE' => ['SEV3', 'S6', false, ['OPERATIONS_MANAGER', 'COMPLIANCE_OFFICER'],
                'A destination institution has not answered a return request within 48 hours. Chase its settlement team, then record the answer (returned, partly returned or refused).'],
            'HOOK_RELEASE_PARTIAL' => ['SEV3', 'S7', false, ['OPERATIONS_MANAGER', 'HEAD_OF_PRODUCTS'],
                'An expired card hook could not be released at every source institution. Call each institution to confirm the hold is gone (it may have expired it already), then record the outcome.'],
