 ...
 * doesn't have one yet (see CardService::provisionUserCard() — no
 * hold placed, no fee charged, just a card row waiting to be
 * activated). Returns the card's status, the QR payload (only
 * meaningful once ACTIVE — an inactive card can't be hooked to, see
 * the hookSourcesToCard() status guard), hook status, and any active
...
// Auto-provision: idempotent, zero-cost, zero-hold. Every user ends up
// with exactly one card, INACTIVE until they pay to activate it.
$provision = $cardService->provisionUserCard($userId, $userName);
if (!($provision['success'] ?? false)) {
    http_response_code(500);
    echo json_encode(['success' => false, 'error' => 'Could not provision a card for this account.']);
    exit();
}
