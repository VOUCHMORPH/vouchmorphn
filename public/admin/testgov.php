...
// Require an ACTUAL method call, not just the word "SwapService" appearing
// anywhere in the file — a comment like "// Call existing SwapService" would
// satisfy a plain substring check while the code below it does nothing real.
...
// Wired means: a real call exists AND the fabricated-reference stub pattern
// is not also present (belt-and-braces — a real implementation should never
// contain both).
...
addResult(
    ...
    $isWired ? 'pass' : 'fail',
    $isWired
        ? 'Real SwapService call found (new SwapService(...) / ->executeAtomicSwap(...) / ->executeMultiDestinationSwap(...)), and no fabricated-reference stub pattern detected'
        : ($isStubbed
            ? 'FAIL — execute.php contains the fabricated-reference stub pattern ($swapReference = \'SWAP_\' . date(...)) and marks every row SUCCESS without ever calling SwapService for real. The word "SwapService" may appear in a comment above this code, which is not the same as calling it — every batch "executed" today has moved no real money.'
            : 'execute.php not found or contains no real SwapService method call'),
    $isWired ? '' : 'Rewrite execute.php to build a payload matching SwapService::executeAtomicSwap() with a destinations[] array — one source (from organization_sources), many destinations (from import_rows) — and write real per-destination results back to payment_instructions instead of assuming blanket success. public/api/v1/swap/execute.php already has a working reference pattern for this exact call in this codebase.'
);
