...
$setupReady = true; // this page is gated to owner/IT roles, always past setup
...
$navItems = [
    ...
    ['key' => 'team', 'icon' => 'idcard', 'label' => 'Team', 'href' => 'users.php', 'show' => $canManageUsers, 'active' => true],
...
<head>
    ...
    <style>
        /* One-time credential display — same brass-accent card treatment
           departments.php uses for its own "show this once" moment, not
           the old full-bleed dark box, but keeping the real copy-to-
           clipboard button and its #tempPwText hook intact. */
        .creds-row { display:flex; gap:16px; flex-wrap:wrap; align-items:center; margin-bottom:6px; font-size:14px; }
        .creds-row .k { color:var(--ink-500); font-family:var(--f-cond); font-size:11px; text-transform:uppercase; letter-spacing:.04em; min-width:90px; }
        .creds-row .v { font-family:var(--f-mono); font-weight:700; font-size:16px; color:var(--ink-900); }
        .creds-copy-btn { background:var(--brass); color:#fff; border:none; padding:4px 12px; font-size:11px; font-weight:700; cursor:pointer; font-family:var(--f-cond); text-transform:uppercase; }
...
<body>
    <?php require __DIR__ . '/../partials/shell-head.php'; ?>
            ...
            <div class="info-panel" style="border-left-color: var(--seal-red); background: var(--danger-bg);">
                <div class="label" style="color:var(--danger);">⚠️ <?php echo safeHtmlU($error); ?></div>
            </div>
            <?php endif; ?>

            <?php if ($newCredentials): ?>
            <div class="card" style="border-left:4px solid var(--brass); background:var(--brass-tint);">
                <div style="color:var(--amber); font-size:12px; font-weight:700; text-transform:uppercase; letter-spacing:.05em; font-family:var(--f-cond); margin-bottom:10px;">⚠ One-time display — copy this now, it will not be shown again</div>
                ...
                <div style="font-size:12px; color:var(--ink-500); margin-top:10px;">
            ...
            <div class="info-panel" style="border-left-color: var(--ledger-green); background: var(--green-tint);">
                <div class="label" style="color:var(--ledger-green);">✅ <?php echo safeHtmlU($success); ?></div>
            </div>
            <?php endif; ?>
