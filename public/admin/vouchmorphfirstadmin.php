...
try {
    $stmt = $db->prepare("
        INSERT INTO admins (
            username, email, role_id, full_name, country_code,
            mfa_enabled, created_at, updated_at
        ) VALUES (
            :username, :email, 999, :name, :country,
            false, NOW(), NOW()
        ) RETURNING admin_id
    ");
    $stmt->execute([
    ...
    // Login secret goes to the separate credentials database, not this
    // admins row.
    CredentialsRepository::fromEnvironment()->createAdminCredential($newId, $hash);

    $db->commit();
} catch (\Throwable $e) {
    $db->rollBack();
    fwrite(STDERR, "Failed to create super_admin: " . $e->getMessage() . "\n");
    exit(1);
}
