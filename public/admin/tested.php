...
if ($cashoutAuth) {
    ...
    foreach ($cashoutAuth as $auth) {
        ...
        echo "      pin_code: {$auth['pin_code']}\n";
        echo "      status: {$auth['status']}\n";
        echo "      code_expiry: {$auth['code_expiry']}\n";
        $authId = $auth['auth_id'];
    }
} else {
    echo "   ❌ NOT FOUND\n";
...
if ($audits) {
    ...
    foreach ($audits as $audit) {
        ...
        echo "      performed_by: {$audit['performed_by']}\n";
        echo "      performed_at: {$audit['performed_at']}\n";
    }
} else {
...
// -----------------------------------------------------------------
// 6.7 swap_vouchers (if any)
// -----------------------------------------------------------------
echo "📋 7. swap_vouchers\n";
if ($swapId) {
    $stmt = $pdo->prepare("
        SELECT * FROM swap_vouchers 
        WHERE swap_id = :swap_id
        ORDER BY created_at DESC
    ");
    $stmt->execute([':swap_id' => $swapId]);
    ...
    if ($vouchers) {
        echo "   ✅ FOUND " . count($vouchers) . " voucher(s)\n";
        foreach ($vouchers as $voucher) {
            echo "      voucher_id: {$voucher['voucher_id']}\n";
            echo "      status: {$voucher['status']}\n";
        }
    } else {
        echo "   ⚠️  No vouchers found\n";
    }
} else {
    echo "   ⏭️  SKIPPED (no swap_id)\n";
}
