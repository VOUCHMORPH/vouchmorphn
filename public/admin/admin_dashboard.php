 ...
 * - Invoice fees to participants using fee_invoices table
...
class HybridSettlementStrategy
{
    ...
    {
        // All tables should already exist from schema
    ...
    // ============================================================
    // FEE INVOICING METHODS - UPDATED to use fee_invoices table
    // ============================================================
    
    /**
     * Invoice participants for fees
     * Now uses the fee_invoices table directly
     */
    public function invoiceFee(
        ...
        try {
            // Insert directly into fee_invoices table
            $stmt = $this->db->prepare("
                INSERT INTO fee_invoices 
                (invoice_uuid, swap_reference, participant_id, source_institution, 
                 fee_type, fee_amount, currency, vat_amount, total_amount, 
                 status, created_at)
                VALUES 
                (:invoice_uuid, :swap_reference, :participant_id, :source_institution,
                 :fee_type, :fee_amount, :currency, :vat_amount, :total_amount,
            ...
            // Also store in settlement_outbox for message delivery
            $invoice = [
                'invoice_uuid' => $invoiceUuid,
                'swap_reference' => $swapReference,
                'fee_type' => $feeType,
            ...
            error_log("[SETTLEMENT] Fee invoice sent to $institution: $totalAmount $currency for $feeType (UUID: $invoiceUuid)");
            
            return $invoiceUuid;
            
        } catch (Exception $e) {
            error_log("[SETTLEMENT] Failed to create fee invoice: " . $e->getMessage());
            throw $e;
        }
    }
     ...
     * Now queries fee_invoices table
     */
    public function getOutstandingInvoices(string $institutionName): array
    {
        try {
            ...
            error_log("[SETTLEMENT] Failed to get outstanding invoices: " . $e->getMessage());
            return [];
        }
    }
    ...
    public function getInvoice(string $invoiceUuid): ?array
    {
        try {
            $stmt = $this->db->prepare("
                ...
                LIMIT 1
            ");
            $stmt->execute([':invoice_uuid' => $invoiceUuid]);
            $result = $stmt->fetch(PDO::FETCH_ASSOC);
            return $result ?: null;
            
        } catch (Exception $e) {
            error_log("[SETTLEMENT] Failed to get invoice: " . $e->getMessage());
            return null;
        }
    }
    
    /**
     * Get all invoices for a swap reference
     */
    public function getInvoicesForSwap(string $swapReference): array
    {
        try {
            $stmt = $this->db->prepare("
                SELECT * FROM fee_invoices 
                WHERE swap_reference = :swap_reference
                ORDER BY created_at DESC
            ");
            $stmt->execute([':swap_reference' => $swapReference]);
            return $stmt->fetchAll(PDO::FETCH_ASSOC);
            
        } catch (Exception $e) {
            error_log("[SETTLEMENT] Failed to get invoices for swap: " . $e->getMessage());
            return [];
        }
    }
    ...
    private function getSettlementsInRange(string $startDate, string $endDate): array
    {
        $stmt = $this->db->prepare("
            ...
            LEFT JOIN fee_invoices fi ON so.swap_reference = fi.swap_reference
