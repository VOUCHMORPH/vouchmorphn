...
.shell-header { position: sticky; top: 0; z-index: 100; background: var(--surface); border-bottom: 1px solid var(--border); }
.shell-header-inner { max-width: var(--max-w); margin: 0 auto; padding: 0 24px; height: var(--header-h); display: flex; align-items: center; justify-content: space-between; gap: 16px; }
.brand { font-size: 17px; font-weight: 800; letter-spacing: -0.01em; }
.brand sup { color: var(--accent); font-size: 9px; }
.back-btn { display: none; align-items: center; gap: 8px; font-size: 12px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.04em; color: var(--text-muted); background: none; border: none; cursor: pointer; padding: 8px 4px; font-family: var(--font); }
.back-btn:hover { color: var(--text); }
.back-btn.show { display: inline-flex; }
...
.user-chip { width: 32px; height: 32px; background: var(--primary); color: #fff; font-size: 11px; font-weight: 700; display: flex; align-items: center; justify-content: center; cursor: pointer; flex-shrink: 0; }
...
#hubView { max-width: 760px; margin: 0 auto; padding: 48px 24px 60px; }
.hub-eyebrow { text-align: center; font-size: 11px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.1em; color: var(--text-dim); font-family: var(--font-mono); margin-bottom: 8px; }
.hub-title { text-align: center; font-size: 25px; font-weight: 700; margin-bottom: 36px; }
.product-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 16px; }
@media (max-width: 560px) { .product-grid { grid-template-columns: 1fr; } }
.product-tile { background: var(--surface); border: 1px solid var(--border); padding: 26px 22px; cursor: pointer; transition: var(--transition); text-align: left; position: relative; }
.product-tile:hover { border-color: var(--primary); transform: translateY(-2px); }
.product-tile-icon { width: 42px; height: 42px; background: var(--primary); display: flex; align-items: center; justify-content: center; margin-bottom: 16px; }
.product-tile-icon svg { width: 21px; height: 21px; stroke: #fff; }
.product-tile-title { font-size: 16px; font-weight: 700; margin-bottom: 5px; display: flex; align-items: center; gap: 8px; }
.product-tile-sub { font-size: 12px; color: var(--text-muted); line-height: 1.5; margin-bottom: 12px; }
.product-tile-arrow { font-size: 12px; color: var(--accent); font-weight: 700; }
.product-tile-badge { font-size: 8px; font-weight: 800; letter-spacing: 0.04em; color: var(--accent); border: 1px solid var(--accent); padding: 2px 6px; text-transform: uppercase; }
...
.cta-row { display: flex; justify-content: center; margin-top: 10px; gap: 10px; }
.cta-row .btn, .cta-row .btn-secondary { flex: 1 1 0; max-width: 360px; }
.btn { padding: 14px 24px; border: none; font-size: 11px; font-weight: 700; font-family: var(--font); cursor: pointer; letter-spacing: 0.08em; text-transform: uppercase; transition: var(--transition); width: 100%; background: var(--primary); color: #fff; }
.btn:hover:not(:disabled) { background: var(--accent); }
.btn:disabled { opacity: 0.4; cursor: not-allowed; }
.btn-primary { background: var(--primary); color: #fff; }
.btn-primary:hover:not(:disabled) { background: var(--accent); }
...
/* Full-screen QR takeover */
.qr-full-wrap { max-width: 420px; margin: 0 auto; padding: 56px 24px; text-align: center; }
.qr-frame-lg { background: #fff; padding: 28px; border: 1px solid var(--border-strong); display: inline-block; margin-bottom: 24px; }
.qr-full-suffix { font-family: var(--font-mono); font-size: 16px; font-weight: 700; letter-spacing: 0.08em; margin-bottom: 6px; }
.qr-full-name { font-size: 13px; color: var(--text-muted); }
.qr-full-hint { font-size: 12px; color: var(--text-muted); margin-top: 20px; }
...
<div class="view" id="hookView">
    <div class="product-view-inner">
        ...
        <div id="hookRowsHolder"></div>
        <button class="add-row-btn" id="addHookRowBtn" style="display:none;" onclick="addHookRow()">+ Add another source</button>
        <button class="btn" onclick="confirmHookBuilder()">Hook to card</button>
        <div style="font-size:11px;color:var(--text-dim);text-align:center;margin-top:14px;">Each source is held for 24 hours per its own authorized amount.</div>
    </div>
</div>
...
function renderToolboxBody() {
    ...
    const sourcesHtml = userSources.length === 0
        ? `<div style="font-size:12px;color:var(--text-dim);padding:8px 0;">No sources linked yet.</div>`
        : userSources.map(source => {
            const isActive = source.status === 'active';
            return `<div class="myc-source-row">
                <div class="myc-source-tile">${escapeHtml(institutionInitials(source.institution))}</div>
                <div class="myc-source-info"><div class="myc-source-inst">${escapeHtml(PARTICIPANTS[source.institution]?.name || source.institution)}</div><div class="myc-source-ident">${escapeHtml(ASSETS[source.asset_type]?.label || source.asset_type)} · ${escapeHtml(source.identifier || source.source_identifier || '')}</div></div>
                <div style="display:flex;flex-direction:column;align-items:flex-end;gap:4px;">
                    ${isActive ? `<button class="unhook-link" style="color:var(--accent);" onclick="useSourceForSwap('${source.id}')">Use</button>` : `<span class="source-status-badge expired">${escapeHtml(source.status)}</span>`}
    ...
    if (isAgent) groups.push({ title: 'Agent', rows: [ { icon: '🧰', label: 'Agent tools', action: 'openAgentToolsModal()' }, { icon: '🏢', label: 'Agent destinations', action: 'openAgentModal()' } ] });
    groups.push({ title: 'Account', rows: [ { icon: '👤', label: 'My profile', action: 'openProfileModal()' }, { icon: '❓', label: 'Help', action: 'openHelpModal()' }, { icon: '📄', label: 'Terms and conditions', action: 'openTermsModal()' } ] });

    const progressHtml = doneCount < steps.length ? `<div style="background:var(--surface-muted);border:1px solid var(--border);padding:12px 14px;margin-bottom:18px;"><div style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:0.04em;color:var(--text-dim);margin-bottom:6px;">Getting set up — ${doneCount}/${steps.length}</div>${steps.map(s => `<div style="font-size:12px;color:${s.done ? 'var(--success)' : 'var(--text-muted)'};padding:2px 0;">${s.done ? '✓' : '○'} ${s.label}</div>`).join('')}</div>` : '';

    const groupsHtml = groups.map(g => `<div class="toolbox-group"><div class="toolbox-group-title">${g.title}</div><div class="toolbox-list">${g.rows.map(r => `<div class="toolbox-row" onclick="${r.action}"><span class="toolbox-row-icon">${r.icon || ''}</span><span class="toolbox-row-label">${r.label}</span>${r.badge ? `<span class="toolbox-badge" style="display:inline-flex;">${r.badge}</span>` : ''}</div>`).join('')}</div></div>`).join('');

    return `
        ${progressHtml}
        <div class="myc-panel">
            <div class="myc-panel-head"><span class="myc-panel-title">My sources</span></div>
            ${sourcesHtml}
            ${identityClaimHtml}
        </div>
        ${groupsHtml}
        <div class="myc-panel">
            <div class="myc-panel-head"><span class="myc-panel-title">Theme</span></div>
            <div class="theme-picker">
                <div class="theme-chip" data-theme="classic" onclick="setTheme('classic')"><div class="swatch"><div class="dot" style="background:#10201C;"></div><div class="dot" style="background:#00A878;"></div></div>Classic</div>
                <div class="theme-chip" data-theme="women" onclick="setTheme('women')"><div class="swatch"><div class="dot" style="background:#7A3B57;"></div><div class="dot" style="background:#E08FA4;"></div></div>Women</div>
                <div class="theme-chip" data-theme="kids" onclick="setTheme('kids')"><div class="swatch"><div class="dot" style="background:#1E3A8A;"></div><div class="dot" style="background:#FFB400;"></div></div>Kids</div>
...
function renderCardQr(payloadText) {
    ...
    if (!el || typeof QRCode === 'undefined') return;
    el.innerHTML = '';
    new QRCode(el, { text: payloadText, width: 140, height: 140 });
}
function goQrFull() {
    if (!myCard || !myCard.qr_payload) { showMessage("Card QR isn't ready yet.", 'warning'); return; }
    document.getElementById('qrFullSuffix').textContent = '•••• ' + myCard.card_suffix;
    ...
    pushView('qrfull');
    document.getElementById('qrHolderLg').innerHTML = '';
    new QRCode(document.getElementById('qrHolderLg'), { text: myCard.qr_payload, width: 240, height: 240 });
}
...
async function confirmHookBuilder() {
    ...
    const result = await callApi(CONFIG.API_BASE + '/api/v1/cards/hook.php', { card_suffix: hookEntry.targetCardSuffix, sources });
    if (!result.ok) { showMessage('That hook didn\'t go through: ' + friendlyApiError(result.error), 'error'); return; }
    showMessage(`Hooked ${sources.length} source(s) for 24 hours. 🎉`, 'success');
    goBack();
    loadCardView();
}
